jQuery(document).ready(function($){
	...
	if($(window).width() > MQL) {
		var headerHeight = $('.jx-header').height();
		$(window).on('scroll',
		{
	        previousTop: 0
	    }, 
	    function () {
		    ...
		    if (currentTop < this.previousTop ) {
		    	//if scrolling up...
		    	if (currentTop > 0 && $('.jx-header').hasClass('is-fixed')) {
		    		$('.jx-header').addClass('is-visible');
		    	} else {
		    		$('.jx-header').removeClass('is-visible is-fixed');
		    	}
		    } else {
		    	//if scrolling down...
		    	$('.jx-header').removeClass('is-visible');
		    	if( currentTop > headerHeight && !$('.jx-header').hasClass('is-fixed')) $('.jx-header').addClass('is-fixed');
		    }
	...
	$('.jx-primary-nav-trigger').on('click', function(){
		...
		$('.jx-header').toggleClass('menu-is-open');
